...
void setup(void) {
  ...
  pinMode(PIN_ir_out, OUTPUT); digitalWrite(PIN_ir_out, LOW);
#ifdef PIN_ir_LED
  pinMode(PIN_ir_LED, OUTPUT); digitalWrite(PIN_ir_LED, LOW);
#endif
#ifdef PIN_ir_VCC
  pinMode(PIN_ir_VCC, OUTPUT); digitalWrite(PIN_ir_VCC, HIGH);
#endif

  //WIFI INIT
#ifdef IPFIX
  WiFi.config(LocalIP, Gateway, Netmask, DNS);
#endif
  ...
  byte mac[6];
  WiFi.macAddress(mac);
  sprintf(host, "esp%02x%02x%02x%02x", mac[2], mac[3], mac[4], mac[5]);
  ...
  server.on("/ir.cgi", HTTP_GET, handleCGI);
  sr_begin();
  server.begin();
